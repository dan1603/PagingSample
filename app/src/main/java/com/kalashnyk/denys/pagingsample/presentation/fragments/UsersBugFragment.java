...
public class UsersBugFragment extends Fragment implements ItemClickListener {
    ...
    private void observersRegisters() {
        ...
        Observer<PagedList<UserEntity>> observer = new Observer<PagedList<UserEntity>>() {
            @Override
            public void onChanged(@Nullable PagedList<UserEntity> userEntities) {
                mPageListAdapter.submitList(userEntities);
            }
        };
        mViewModel.getBugUsers().observeForever(observer);
    ...
    public void onDestroy() {
        super.onDestroy();
        mViewModel.getBugUsers().removeObservers(this);
    }
}